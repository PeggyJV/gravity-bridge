...
impl CosmosSender {
    ...
    pub async fn update_gravity_delegate_addresses(
        ...
        let our_valoper_address = self.cosmos_key
            ...
            .to_bech32(format!("{}valoper", self.contact.get_prefix()))
            .unwrap();

        let nonce = self.contact
            .get_account_info(self.cosmos_key.to_address(&self.contact.get_prefix()).unwrap())
            .await?
            .sequence;
    ...
    pub async fn send_to_eth(
        ...
        let cosmos_address = self.cosmos_key.to_address(&self.contact.get_prefix()).unwrap();
    ...
    pub async fn send_messages(
        ...
        let mut args = self.contact.get_message_args(cosmos_address, fee).await?;

        let tx_parts = self.cosmos_key.build_tx(&messages, args.clone(), MEMO)?;
        let gas = self.contact.simulate_tx(tx_parts).await?;

        // multiply the estimated gas by the configured gas adjustment
        let gas_limit: f64 = (gas.gas_used as f64) * self.gas_adjustment;
        args.fee.gas_limit = cmp::max(gas_limit as u64, 500000 * messages.len() as u64);

        // compute the fee as fee=ceil(gas_limit * gas_price)
        let fee_amount: f64 = args.fee.gas_limit as f64 * self.gas_price.0;
        ...
        let msg_bytes = self.cosmos_key.sign_std_msg(&messages, args, MEMO)?;
        let response = self.contact
            .send_transaction(msg_bytes, BroadcastMode::Sync)
            .await?;

        Ok(self.contact.wait_for_tx(response, TIMEOUT).await?)
    }
    ...
    pub async fn send_in_batches(
        ...
        for msg_chunk in messages.chunks(self.msg_batch_size) {
            let batch = msg_chunk.to_vec();
            match self.send_messages(batch.clone())
            .await
            {
                Ok(res) => debug!("message batch sent: {:?}", res),
                Err(err) => {
                    self.log_send_error(&batch, err);
                    ...
                    for msg in batch {
                        let msg_vec = vec![msg];
                        match self.send_messages(msg_vec.clone())
                        .await
                        {
                            Ok(res) => debug!("message sent: {:?}", res),
                            Err(err) => {self.log_send_error(&msg_vec, err)}
                        }
                    }
                }
            }
    ...
    pub async fn wait_for_tx(&self, response: TxResponse, timeout: Duration) -> Result<TxResponse, CosmosGrpcError> {
        return self.contact.wait_for_tx(response, timeout).await
    }

    fn log_send_error(&self, messages: &Vec<Msg>, err: GravityError) {
