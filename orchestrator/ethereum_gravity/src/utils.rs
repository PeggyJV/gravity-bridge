...
pub fn extract_gravity_contract_error(gravity_error: GravityError) -> Option<GravityContractError> {
    match gravity_error {
        GravityError::EthersContractError(ethers::contract::ContractError::MiddlewareError(
            ethers::middleware::signer::SignerMiddlewareError::MiddlewareError(
                    ...
                    ethers::providers::HttpClientError::JsonRpcError(jre) => {
                        if jre.code == 3 && jre.data.is_some() {
                            ...
                            if data.is_string() {
                                ...
                                match data_bytes {
                                    Ok(data_bytes) => decode_gravity_error(data_bytes),
                                    Err(_) => None,
                                }
                            } else {
                                None
