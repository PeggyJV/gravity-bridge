...
func (k Keeper) createSendToEthereum(ctx sdk.Context, chainID uint32, sender sdk.AccAddress, counterpartReceiver string, amount sdk.Coin, fee sdk.Coin) (uint64, error) {
	...
	nextID := k.incrementLastSendToEthereumIDKey(ctx, chainID)
	...
	k.setUnbatchedSendToEthereum(ctx, chainID, &types.SendToEthereum{
		Id:                nextID,
...
func (k Keeper) cancelSendToEthereum(ctx sdk.Context, chainID uint32, id uint64, s string) error {
	sender, _ := sdk.AccAddressFromBech32(s)

	var send *types.SendToEthereum
	for _, ste := range k.getUnbatchedSendToEthereums(ctx, chainID) {
		if ste.Id == id {
			send = ste
		}
	}
	...
	k.deleteUnbatchedSendToEthereum(ctx, chainID, send.Id, send.Erc20Fee)
	return nil
}

func (k Keeper) setUnbatchedSendToEthereum(ctx sdk.Context, chainID uint32, ste *types.SendToEthereum) {
	ctx.KVStore(k.storeKey).Set(types.MakeSendToEVMKeyForEvent(chainID, ste.Id, ste.Erc20Fee), k.cdc.MustMarshal(ste))
}

func (k Keeper) deleteUnbatchedSendToEthereum(ctx sdk.Context, chainID uint32, id uint64, fee types.ERC20Token) {
